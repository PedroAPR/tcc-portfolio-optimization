...
O resultado final é a distribuição *a posteriori*, calculada aplicando a Regra de Bayes. O vetor de retornos esperados BL ($E\_{BL}$) é uma média ponderada complexa entre o Prior (Equilíbrio) e a Likelihood (Visões).
...
$$\\max\_w U \= w^T \\Pi \- \\frac{\\lambda}{2} w^T \\Sigma w$$  
Onde:
...
$$\\Pi \= \\lambda \\Sigma w\_{mkt}$$  
Este vetor $\\Pi$ representa os retornos que o mercado *precisa* esperar para que os atuais pesos de capitalização ($w\_{mkt}$) sejam ótimos. O parâmetro de aversão ao risco global $\\lambda$ é frequentemente calibrado como $\\lambda \= \\frac{E \- R\_f}{\\sigma^2\_m}$, situando-se tipicamente entre 2 e 4 em estudos empíricos.16
...
* **Visão Absoluta:** Para uma visão sobre o Ativo A (índice $i$), o elemento $P\_{k,i} \= 1$ e os demais são 0\.  
...
É um vetor coluna $K \\times 1$. Cada elemento $Q\_k$ representa o retorno esperado da visão $k$. Para visões relativas, $Q\_k$ é o *spread* ou diferencial de retorno esperado, não o retorno total absoluto.22
...
$$\\Omega \= \\text{diag}(\\omega\_1, \\omega\_2,..., \\omega\_k)$$

A magnitude de $\\omega\_k$ representa a incerteza da visão $k$. Se $\\omega\_k \\to 0$, o investidor tem certeza absoluta (confiança infinita), e o modelo forçará o portfólio a satisfazer a visão exatamente. Se $\\omega\_k \\to \\infty$, a visão é ignorada.17

### **3.3.3 A Fórmula Mestra de Black-Litterman**

Combinando o Prior ($N(\\Pi, \\tau\\Sigma)$) e a Likelihood ($N(Q, \\Omega)$) via Teorema de Bayes, obtemos a distribuição Posterior $\\mu\_{BL} \\sim N(E\_{BL}, \\Sigma\_{BL})$. O vetor de retornos esperados combinados é dado pela "Fórmula Mestra":
...
$$E\_{BL} \= \\Pi \+ \\tau\\Sigma P^T^{-1} \[Q \- P\\Pi\]$$  
...
Note que $\\Sigma\_{BL}$ é maior que a covariância histórica $\\Sigma$. O modelo adiciona uma camada extra de risco, refletindo a incerteza epistêmica sobre a verdadeira média dos retornos.22
...
1. **A Visão Original (Black & Litterman, 1992):** Os autores sugeriram que $\\tau$ deveria ser um valor pequeno (próximo de zero), argumentando que a incerteza sobre a média de longo prazo é muito menor que a volatilidade dos retornos. Valores entre 0,025 e 0,05 são comuns nesta abordagem.24  
2. **A Abordagem Estatística (Walters, 2014; Meucci, 2005):** Argumentam que, se o Prior é derivado de uma série histórica, $\\tau$ deve ser calibrado como o erro padrão da média, ou seja, $\\tau \\approx 1/T$, onde $T$ é o número de observações da amostra. Para 5 anos de dados mensais, $\\tau \\approx 1/60$. Esta visão fornece uma base empírica objetiva para o parâmetro.24  
3. **A Abordagem de Satchell e Scowcroft (2000):** Propuseram fixar $\\tau \= 1$. Embora simplifique a álgebra, essa escolha altera drasticamente o peso relativo do Prior, exigindo que a matriz $\\Omega$ seja recalibrada proporcionalmente para evitar que as visões dominem completamente o portfólio. Eles tratam a incerteza do prior e das visões como magnitudes comparáveis *a priori*.24

Em última análise, como demonstrado por **Thomas Idzorek (2005)**, a escolha do valor escalar de $\\tau$ torna-se irrelevante para o cálculo do vetor de retornos ($E\_{BL}$) se a matriz $\\Omega$ for calibrada endogenamente proporcional a $\\tau$. Contudo, $\\tau$ continua a afetar a matriz de covariância posterior $\\Sigma\_{BL}$, influenciando a magnitude absoluta do risco estimado.17
...
1. **Cálculo do Portfólio de Certeza Total:** O modelo calcula qual seria o vetor de retornos se o investidor tivesse 100% de confiança na visão (o que equivaleria a $\\omega\_k \\approx 0$). Isso gera um vetor de pesos de alocação de "certeza total".  
2. **Determinação do Desvio Máximo:** Calcula-se a diferença de alocação (vetor de *tilts*) entre o portfólio de equilíbrio (sem visões) e o portfólio de certeza total.  
3. **Interpolação Linear pela Confiança:** Se o investidor declara 50% de confiança, o algoritmo define que o *tilt* alvo deve ser 50% do desvio máximo calculado no passo anterior.  
4. **Engenharia Reversa de $\\Omega$:** O algoritmo resolve iterativamente ou analiticamente para encontrar os valores da diagonal de $\\Omega$ que, quando inseridos na fórmula mestra do BL, resultam exatamente nesses pesos-alvo interpolados.

A fórmula implícita derivada por Idzorek assume que a variância da visão é proporcional à variância do portfólio da visão ($p\_k \\Sigma p\_k^T$) ajustada por um fator de escala $\\alpha$ derivado da confiança ($C$):

$$\\omega\_k \= \\tau \\cdot (p\_k \\Sigma p\_k^T) \\cdot \\left(\\frac{1-C}{C}\\right)$$
...
1. **Prior Genérico:** O Prior não precisa ser normal ou de equilíbrio. Pode ser uma distribuição empírica histórica, uma distribuição de Monte Carlo com caudas pesadas, ou derivada de Cópulas para modelar dependência não-linear nas caudas.  
2. **Visões Flexíveis:** O gestor não se limita a visões sobre médias ($Q$). É possível inserir visões sobre volatilidade ("A vol vai aumentar"), correlação, ou medidas de cauda como o VaR ("O risco de perda máxima será de 15%").  
3. **Consistência:** O método garante que a distribuição Posterior seja a mais próxima possível do Prior (preservando a estrutura de mercado) enquanto satisfaz as restrições impostas pelas visões complexas.
...
1. Entrega\_2\_14\_11\_25\_Pedro\_Reis\_TMP.docx  
2. Reading on Black-Litterman Model \- Medium, acessado em dezembro 2, 2025, [https://medium.com/@li.ying.explore/reading-on-black-litterman-model-beecfcd0a82c](https://medium.com/@li.ying.explore/reading-on-black-litterman-model-beecfcd0a82c)  
3. Innovative Black-Litterman Global Asset Allocation Model Is Developed at Goldman Sachs, acessado em dezembro 2, 2025, [https://www.goldmansachs.com/our-firm/history/moments/1990-black-litterman-model](https://www.goldmansachs.com/our-firm/history/moments/1990-black-litterman-model)  
4. Advanced Portfolio Optimization: The Black-Litterman Model & AI Techniques \- MarketXLS, acessado em dezembro 2, 2025, [https://marketxls.com/blog/advanced-portfolio-optimization-black-litterman-ai](https://marketxls.com/blog/advanced-portfolio-optimization-black-litterman-ai)  
5. The Black-Litterman Model In Detail, acessado em dezembro 2, 2025, [https://par.cse.nsysu.edu.tw/resource/paper/2011/110615/The%20Black-Litterman%20Model%20in%20Detail.pdf](https://par.cse.nsysu.edu.tw/resource/paper/2011/110615/The%20Black-Litterman%20Model%20in%20Detail.pdf)  
6. Michaud 1989 | PDF | Modern Portfolio Theory | Mathematical Optimization \- Scribd, acessado em dezembro 2, 2025, [https://www.scribd.com/document/673001364/michaud-1989](https://www.scribd.com/document/673001364/michaud-1989)  
7. The Markowitz Optimization Enigma: is 'Optimized' Optimal? | Request PDF \- ResearchGate, acessado em dezembro 2, 2025, [https://www.researchgate.net/publication/314444182\_The\_Markowitz\_Optimization\_Enigma\_is\_'Optimized'\_Optimal](https://www.researchgate.net/publication/314444182_The_Markowitz_Optimization_Enigma_is_'Optimized'_Optimal)  
8. The Markowitz Optimization Enigma \- ResearchGate, acessado em dezembro 2, 2025, [https://www.researchgate.net/profile/Richard-Michaud-2/publication/247883727\_The\_Markowitz\_Optimization\_Enigma\_Is\_'Optimized'\_Optimal/links/5aba6245a6fdcc0e3d9f69a3/The-Markowitz-Optimization-Enigma-Is-Optimized-Optimal.pdf](https://www.researchgate.net/profile/Richard-Michaud-2/publication/247883727_The_Markowitz_Optimization_Enigma_Is_'Optimized'_Optimal/links/5aba6245a6fdcc0e3d9f69a3/The-Markowitz-Optimization-Enigma-Is-Optimized-Optimal.pdf)  
9. (PDF) Incorporating estimation errors into portfolio selection: Robust portfolio construction, acessado em dezembro 2, 2025, [https://www.researchgate.net/publication/228380747\_Incorporating\_estimation\_errors\_into\_portfolio\_selection\_Robust\_portfolio\_construction](https://www.researchgate.net/publication/228380747_Incorporating_estimation_errors_into_portfolio_selection_Robust_portfolio_construction)  
10. Markowitz mean-variance optimization as "error maximization", acessado em dezembro 2, 2025, [https://quant.stackexchange.com/questions/4132/markowitz-mean-variance-optimization-as-error-maximization](https://quant.stackexchange.com/questions/4132/markowitz-mean-variance-optimization-as-error-maximization)  
11. The Black-Litterman Model Misuses and Abuses “Portfolio Management is very easy … if you don't know what you are doing. L \- Lumen Global Investments, acessado em dezembro 2, 2025, [https://www.lumenglobalinv.com/wp-content/uploads/2021/04/Black\_Litterman\_misuses\_and\_abuses\_April-2021.pdf](https://www.lumenglobalinv.com/wp-content/uploads/2021/04/Black_Litterman_misuses_and_abuses_April-2021.pdf)  
12. The Application of the Black-Litterman model in a Multi-Factor Framework, acessado em dezembro 2, 2025, [https://research.cbs.dk/files/60748109/206982\_Thesis\_final.pdf](https://research.cbs.dk/files/60748109/206982_Thesis_final.pdf)  
13. Black-Litterman Model \- Definition, Example, Formula, Pros n Cons \- Financial Edge, acessado em dezembro 2, 2025, [https://www.fe.training/free-resources/portfolio-management/black-litterman-model/](https://www.fe.training/free-resources/portfolio-management/black-litterman-model/)  
14. Deconstructing Black-Litterman Optimization: A Brief Overview, acessado em dezembro 2, 2025, [https://newfrontieradvisors.com/insights/all-insights/deconstructing-black-litterman-optimization-a-brief-overview/](https://newfrontieradvisors.com/insights/all-insights/deconstructing-black-litterman-optimization-a-brief-overview/)  
15. Bayesian Portfolio Optimisation: Introducing the Black-Litterman Model \- Hudson & Thames, acessado em dezembro 2, 2025, [https://hudsonthames.org/bayesian-portfolio-optimisation-the-black-litterman-model/](https://hudsonthames.org/bayesian-portfolio-optimisation-the-black-litterman-model/)  
16. A STEP-BY-STEP GUIDE TO THE BLACK-LITTERMAN MODEL \- Global Risk Guard, acessado em dezembro 2, 2025, [https://globalriskguard.com/resources/assetman/assetall\_0004.pdf](https://globalriskguard.com/resources/assetman/assetall_0004.pdf)  
17. Black-Litterman Portfolio Optimization Using Financial Toolbox \- MATLAB & Simulink Example \- MathWorks, acessado em dezembro 2, 2025, [https://www.mathworks.com/help/finance/black-litterman-portfolio-optimization.html](https://www.mathworks.com/help/finance/black-litterman-portfolio-optimization.html)  
18. Black-Litterman Allocation — PyPortfolioOpt 1.5.4 documentation, acessado em dezembro 2, 2025, [https://pyportfolioopt.readthedocs.io/en/latest/BlackLitterman.html](https://pyportfolioopt.readthedocs.io/en/latest/BlackLitterman.html)  
19. The Black-Litterman Model: An Investigation of Confidence \- Lund University Publications, acessado em dezembro 2, 2025, [https://lup.lub.lu.se/student-papers/record/9061456/file/9061457.pdf](https://lup.lub.lu.se/student-papers/record/9061456/file/9061457.pdf)  
20. A step-by-step guide to the Black-Litterman model: Incorporating user-specified confidence levels | Request PDF \- ResearchGate, acessado em dezembro 2, 2025, [https://www.researchgate.net/publication/237282015\_A\_step-by-step\_guide\_to\_the\_Black-Litterman\_model\_Incorporating\_user-specified\_confidence\_levels](https://www.researchgate.net/publication/237282015_A_step-by-step_guide_to_the_Black-Litterman_model_Incorporating_user-specified_confidence_levels)  
21. A STEP-BY-STEP GUIDE TO THE BLACK-LITTERMAN MODEL Incorporating user-specified confidence levels \- Duke People, acessado em dezembro 2, 2025, [https://people.duke.edu/\~charvey/Teaching/BA453\_2006/Idzorek\_onBL.pdf](https://people.duke.edu/~charvey/Teaching/BA453_2006/Idzorek_onBL.pdf)  
22. Black-Litterman Model, acessado em dezembro 2, 2025, [https://www.stat.berkeley.edu/\~nolan/vigre/reports/Black-Litterman.pdf](https://www.stat.berkeley.edu/~nolan/vigre/reports/Black-Litterman.pdf)  
23. Black-Litterman allocation model: Application and comparision with OMX Stockholm Benchmark PI (OMXSBPI), acessado em dezembro 2, 2025, [https://lup.lub.lu.se/luur/download?func=downloadFile\&recordOId=5472145\&fileOId=5472158](https://lup.lub.lu.se/luur/download?func=downloadFile&recordOId=5472145&fileOId=5472158)  
24. The Factor Tau in the Black-Litterman Model \- ResearchGate, acessado em dezembro 2, 2025, [https://www.researchgate.net/publication/228317768\_The\_Factor\_Tau\_in\_the\_Black-Litterman\_Model](https://www.researchgate.net/publication/228317768_The_Factor_Tau_in_the_Black-Litterman_Model)  
25. What is the tau parameter in the Black-Litterman model?, acessado em dezembro 2, 2025, [https://quant.stackexchange.com/questions/12651/what-is-the-tau-parameter-in-the-black-litterman-model](https://quant.stackexchange.com/questions/12651/what-is-the-tau-parameter-in-the-black-litterman-model)  
26. Black–Litterman Portfolio Optimization with Dynamic CAPM via ABC-MCMC \- MDPI, acessado em dezembro 2, 2025, [https://www.mdpi.com/2227-7390/13/20/3265](https://www.mdpi.com/2227-7390/13/20/3265)  
27. The Black–Litterman Model for Active Portfolio Management \- ResearchGate, acessado em dezembro 2, 2025, [https://www.researchgate.net/publication/247906188\_The\_Black-Litterman\_Model\_for\_Active\_Portfolio\_Management](https://www.researchgate.net/publication/247906188_The_Black-Litterman_Model_for_Active_Portfolio_Management)  
28. Enhancing Black-Litterman Portfolio via Hybrid Forecasting Model Combining Multivariate Decomposition and Noise Reduction \- arXiv, acessado em dezembro 2, 2025, [https://arxiv.org/html/2505.01781v2](https://arxiv.org/html/2505.01781v2)  
29. Enhancing Black-Litterman Portfolio via Hybrid Forecasting Model Combining Multivariate Decomposition and Noise Reduction \- arXiv, acessado em dezembro 2, 2025, [https://arxiv.org/html/2505.01781v1](https://arxiv.org/html/2505.01781v1)  
30. Entropy Pooling vs Black-Litterman \- Medium, acessado em dezembro 2, 2025, [https://medium.com/@ft\_anvo/entropy-pooling-vs-black-litterman-abb608b810cd](https://medium.com/@ft_anvo/entropy-pooling-vs-black-litterman-abb608b810cd)  
31. Fully flexible extreme views \- libra@unine.ch, acessado em dezembro 2, 2025, [https://libra.unine.ch/bitstreams/b392f092-7a14-4d8c-a443-9b10302d3413/download](https://libra.unine.ch/bitstreams/b392f092-7a14-4d8c-a443-9b10302d3413/download)  
32. A Short Review over Twenty Years on the Black-Litterman Model in Portfolio Optimization, acessado em dezembro 2, 2025, [http://iemsjl.org/journal/article.php?code=81826](http://iemsjl.org/journal/article.php?code=81826)  
33. General CoVaR Based on Entropy Pooling \- arXiv, acessado em dezembro 2, 2025, [https://arxiv.org/pdf/2509.21904](https://arxiv.org/pdf/2509.21904)  
34. Beyond Black-Litterman: Views on Non-Normal Markets | Request PDF \- ResearchGate, acessado em dezembro 2, 2025, [https://www.researchgate.net/publication/228205725\_Beyond\_Black-Litterman\_Views\_on\_Non-Normal\_Markets](https://www.researchgate.net/publication/228205725_Beyond_Black-Litterman_Views_on_Non-Normal_Markets)